...
const sendEmailBasedOnDocumentKey = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const exportation = yield (0, exportation_dynamo_core_1.getExportationByNumeroDo)(numero_do);
        const items = exportation.Items;
        switch (req.params.method) {
            case 'initial':
                if (items != undefined) {
                    const fileRUrl = yield (0, s3_core_1.getSignedURL)(items[0]["booking"]);
                    const fileFurl = yield (0, s3_core_1.getSignedURL)(items[0]["factura"]);
                    const fileCPurl = yield (0, s3_core_1.getSignedURL)(items[0]["CP"]);
                    const message = yield (0, sendEmails_core_1.returnMessageInitialDocuments)(fileRUrl, fileFurl, fileCPurl, numero_do);
            ...
            case 'SAE':
                if (items != undefined) {
                    ...
                    const message = yield (0, sendEmails_core_1.returnMessageSaex)(fileSaexUrl, numero_do);
                    yield (0, sendEmails_core_1.sendMessage)(message);
                    res.json({ "Mensaje": "correo enviado" });
                }
                else {
            ...
            case 'booking':
                if (items != undefined) {
                    const fileUrl = yield (0, s3_core_1.getSignedURL)(items[0]["booking"]);
                    const message = yield (0, sendEmails_core_1.returnUpdateDocument)(fileUrl, numero_do, 'booking');
                    yield (0, sendEmails_core_1.sendMessage)(message);
                    res.json({ "Mensaje": "correo enviado" });
                }
                else {
            ...
            case 'factura':
                if (items != undefined) {
                    const fileUrl = yield (0, s3_core_1.getSignedURL)(items[0]["factura"]);
                    const message = yield (0, sendEmails_core_1.returnUpdateDocument)(fileUrl, numero_do, 'factura');
                    yield (0, sendEmails_core_1.sendMessage)(message);
                    res.json({ "Mensaje": "correo enviado" });
                }
                else {
            ...
            case 'CP':
                if (items != undefined) {
                    const fileUrl = yield (0, s3_core_1.getSignedURL)(items[0]["CP"]);
                    const message = yield (0, sendEmails_core_1.returnUpdateDocument)(fileUrl, numero_do, 'CP');
                    yield (0, sendEmails_core_1.sendMessage)(message);
                    res.json({ "Mensaje": "correo enviado" });
                }
                else {
